...
const ProgressBar = ({ progressBarRef, audioRef, timeProgress, duration }: ProgressBarProps) => {
  ...
  return (
    <div className={progressBarStyles["progress"]}>
      <span className={`${progressBarStyles["time"]} ${progressBarStyles["current"]}`}>{formatTime(timeProgress)}</span>
      <input type="range" ref={progressBarRef} defaultValue={0} onChange={handleProgressChange} />
      <span className={progressBarStyles["time"]}>{formatTime(duration)}</span>
    </div>)
}
